...
RNNmodel = torch.load("RNN3.pt",map_location=device)
RNNmodel.eval()
NNmodel = torch.load("NN3.pt",map_location=device)
NNmodel.eval()
NBmodel=None
SVMmodel=None
...
class BookGUI:
    ...
    def get_classNN(self):
        ...
        d_vectors = torch.tensor(d_vectors.toarray(),dtype=torch.float32).to(device)
